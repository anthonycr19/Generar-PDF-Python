...
def generar_pdf(request, ubigeo, scr_inic):
    ...
    for aeu_v in caso:
        ...
        viviendas_totales = Vw_Seg_Esp_R_Secdist_Secc.objects.filter(idscr=idscr).order_by('codccpp')
        ...
        data = [
            ...
            [Paragraph('<strong>PROVINCIA</strong>', h1), Paragraph(distrito.ccpp, h_center),Paragraph(str(distrito.cod_prov.provincia).decode('latin-1'), h1), '', '',''],
            [Paragraph('<strong>DISTRITO</strong>', h1), Paragraph(distrito.ccdi, h_center), Paragraph(str(distrito.distrito).decode('latin-1'), h1),'', Paragraph('<strong>C. TOTAL DE VIVIENDAS DEL AER.</strong>', h1),Paragraph(str(total_viv), h1)],
        ...
        viviendas = Vw_Seg_Esp_R_Secdist_Secc.objects.filter(idscr=idscr).order_by('codccpp')
...
def generar_lote(request):
    ...
    for ubigeos in range(len(lista_distrito)):
        total_secciones = int(str(Seg_R_Secc_Ccpp.objects.filter(ubigeo=lista_distrito[ubigeos]).values_list('scr_ini', flat=True).distinct().count()))
        ...
        for pos in range(total_secciones):
            ...
            generar_pdf(request,lista_distrito[ubigeos],uchi[pos])
